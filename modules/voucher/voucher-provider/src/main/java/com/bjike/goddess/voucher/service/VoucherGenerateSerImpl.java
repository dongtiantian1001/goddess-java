...
//        String startTime = year + "-01-01";
        ...
        if (year != 1) {
            String beginStartTime = sTime.substring(0,4) + "-01-01";
            String beginEndTime = TemporalAdjusters.lastDayOfMonth().toString();
//            System.out.println(beginEndTime);
...
//                    DateUtil.getDayByDate(year, DateUtil.parseDate(sTime).getMonthValue())));
        ...
        String eStartTime = sTime.substring(0,4)+ "-01-01";
    ...
    public List<String> findAllArea() throws SerException {
        ...
        sb.append("select distinct area from voucher_vouchergenerate");
        String file[] =new String []{"area"};
        List<VoucherGenerate> voucherGenerateList = super.findBySql(sb.toString(),VoucherGenerate.class,file);
        //List<VoucherGenerate> voucherGenerateList = super.findAll();
        if (null != voucherGenerateList && voucherGenerateList.size() > 0) {
            List<String> list = voucherGenerateList.stream().map(VoucherGenerate::getArea).distinct().collect(Collectors.toList());
            return list;
        }
        return null;
    }
    ...
    public List<String> findAllGroup() throws SerException {
        StringBuffer sb =new StringBuffer();
        sb.append("select distinct projectGroup from voucher_vouchergenerate");
        String file[] =new String []{"projectGroup"};
        List<VoucherGenerate> voucherGenerateList = super.findBySql(sb.toString(),VoucherGenerate.class,file);
        if (null != voucherGenerateList && voucherGenerateList.size() > 0) {
            List<String> list = voucherGenerateList.stream().map(VoucherGenerate::getProjectGroup).distinct().collect(Collectors.toList());
            return list;
        }
        return null;
    }
